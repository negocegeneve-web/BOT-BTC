/* ============================================================
 *  SERVEUR 3.14 - CADRE R BIMODAL  (bonus armable / filtre carnet / cap 8 positions)
 *  ------------------------------------------------------------
...
function managePosition(symbol) {
  ...
  if (pos.bonus) {
    ...
    if (pos.bonusPeakGain >= STRAT.BONUS_TP_ARM_STAKE) {
      pos.bonusArmed = true; // 3.14 : seuil d'armement atteint -> trailing actif
  ...
  const partAt = vol ? STRAT.VOL_PARTIAL_AT_R : STRAT.PARTIAL_AT_R;   // 0.4R vs 1R
  ...
  if (!pos.partialDone && pnlPct >= partAt * rPct && pos.qty > 0) {
    ...
    if (chunk > 0 && chunk < pos.qty) {
      ...
      closePos(symbol, vol ? 'PARTIEL+0.4R' : 'PARTIEL+1R', chunk);
...
<body>
  <div class="head">
    ...
    <span class="badge" style="background:rgba(0,245,200,.12);color:#00F5C8;border:1px solid rgba(0,245,200,.3)">3.14 - Cadre R bimodal · 40 sym · 8 pos <span style="opacity:.6;font-weight:600">· WR ~49% · +0.6R/trade</span></span>
    <span id="mode" class="badge net">TESTNET</span>
    <span id="run" class="badge off">PAUSE</span>
  </div>
  <div class="sub" id="stratline">3.14-Cadre R bimodal · 8 positions · bonus armé à +27% mise · filtre carnet 3:1 · maker patient en RANGE · anti-429 · mode volatil 1R>5% (0.4R/60%/-0.6R/0.25R) · risque/trade ≤1.2% · stop 1.5 ATR · partiel 50% +1R · trail 0.5R · paliers de mise · garde friction · verrou d'entrée · bonus Q70+ x9 · zéro rotation</div>
...
async function start() {
  logLine(`\u{1F680} Itachi — SERVEUR 3.14-Cadre R BIMODAL (bonus armable +27% mise / filtre carnet 3:1 / cap 8 positions — decrets Calvin 30/08) — ${MODE.toUpperCase()} — capital $${CAPITAL_START}`);
  logLine(`\u{1F4C8} 3.14-Cadre R BIMODAL — cadre R inchange — + bonus trailing arme a +27% de mise (etait +100%, inatteignable) + filtre carnet 3:1 + cap 8 positions (garde marge = arbitre reel)`);
