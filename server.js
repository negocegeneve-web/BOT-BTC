/* ============================================================
 *  SERVEUR 3.14f - CADRE R TRIMODAL  (autotest de connexion / plus de cooldown sur panne d'auth)
 *  ------------------------------------------------------------
...
const STRAT = {
  ...
  STAKE_PALIERS: [      // [capital_max, mise_min, mise_max] — decret Calvin 03/09 : palier 1 (50-100$) SUPPRIME, on demarre au palier 100-175$
    [2200, 100, 175], [4000, 175, 290], [5000, 290, 445],
  ...
  // --- Positions & risque ---
  MAX_POSITIONS_CAP: 8, // decret Calvin 30/08/2026 : 6 -> 8 positions (la garde marge reste l'arbitre reel ; a 950$ la 7e/8e seront souvent refusees faute de marge, c'est voulu)
...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/stats.csv') {
    ...
    const rows = [(['n', 'ferme_le', 'symbole', 'sens', 'levier', 'entree', 'sortie', 'investi_usd',
      'pnl_pct', 'brut_usd', 'frais_usd', 'net_usd', 'duree_min', 'raison', 'Q']).join(',')];
...
<body>
  <div class="head">
    ...
    <span class="badge" style="background:rgba(0,245,200,.12);color:#00F5C8;border:1px solid rgba(0,245,200,.3)">3.14f - Cadre R trimodal · 40 sym · 8 pos <span style="opacity:.6;font-weight:600">· WR ~49% · +0.6R/trade</span></span>
    <span id="mode" class="badge net">TESTNET</span>
    <span id="run" class="badge off">PAUSE</span>
  </div>
  <div class="sub" id="stratline">3.14f-Cadre R trimodal · autotest de connexion · clés nettoyées · paliers dès $100-175 · chien de garde stop natif · anti-fantômes (confirmation 2× + P&L réel Binance) · partiel RANGE +0.5R / tendance +1R / volatil +0.4R · 8 positions · bonus : partiel +15% puis break-even, verrou 6h après perte · filtre carnet 3:1 · maker patient en RANGE · anti-429 · mode volatil 1R>5% (0.4R/60%/-0.6R/0.25R) · risque/trade ≤1.2% · stop 1.5 ATR · partiel 50% +1R · trail 0.5R · paliers de mise · garde friction · verrou d'entrée · bonus Q70+ x9 · zéro rotation</div>
  ...
  <div class="table-wrap"><table>
    <thead><tr><th>#</th><th>Symbole</th><th>Sens</th><th>Lev</th><th>Entrée</th><th>Sortie</th><th>Investi</th><th>P&L%</th><th>Brut</th><th>Frais Binance</th><th>Net retirable</th><th>⏱ Durée</th><th>Fermé le</th><th>Raison</th></tr></thead>
...
<script>
  ...
  function renderStats(s){
    ...
    $('stratline').textContent='3.14f-Cadre R trimodal · autotest de connexion · clés nettoyées · paliers dès $100-175 · chien de garde stop natif · anti-fantômes (confirmation 2× + P&L réel Binance) · partiel RANGE +0.5R / tendance +1R / volatil +0.4R · 8 positions max · bonus : partiel +15% puis break-even, verrou 6h après perte · filtre carnet 3:1 · maker patient 40s en RANGE · anti-429 · mode volatil 1R>5% (partiel 0.4R·60% / stop -0.6R / trail 0.25R) · risque/trade ≤1.2% cap · stop 1.5×ATR(14) · partiel 50% à +1R puis break-even · trail 0.5R · paliers de mise par capital + modulation ATR · garde friction 6× · bonus loterie Q70+ 25-50$ x9 SL-5% · zéro rotation · multi-régime · x2-5';
      if($('connInfo')){
  ...
  function renderTrades(list){
    const tb=$('trades');
    if(!list||!list.length){tb.innerHTML='<tr><td colspan="14" class="mut" style="text-align:center;padding:14px">Aucun trade</td></tr>';return;}
