/* ============================================================
 *  SERVEUR 3.14e - CADRE R TRIMODAL  (fix -2014 : nettoyage agressif des cles API)
 *  ------------------------------------------------------------
...
<body>
  <div class="head">
    ...
    <span class="badge" style="background:rgba(0,245,200,.12);color:#00F5C8;border:1px solid rgba(0,245,200,.3)">3.14e - Cadre R trimodal · 40 sym · 8 pos <span style="opacity:.6;font-weight:600">· WR ~49% · +0.6R/trade</span></span>
    <span id="mode" class="badge net">TESTNET</span>
    <span id="run" class="badge off">PAUSE</span>
  </div>
  <div class="sub" id="stratline">3.14e-Cadre R trimodal · clés nettoyées (fix -2014) · paliers dès $100-175 · chien de garde stop natif · anti-fantômes (confirmation 2× + P&L réel Binance) · partiel RANGE +0.5R / tendance +1R / volatil +0.4R · 8 positions · bonus : partiel +15% puis break-even, verrou 6h après perte · filtre carnet 3:1 · maker patient en RANGE · anti-429 · mode volatil 1R>5% (0.4R/60%/-0.6R/0.25R) · risque/trade ≤1.2% · stop 1.5 ATR · partiel 50% +1R · trail 0.5R · paliers de mise · garde friction · verrou d'entrée · bonus Q70+ x9 · zéro rotation</div>
...
async function start() {
  logLine(`\u{1F680} Itachi — SERVEUR 3.14e-Cadre R TRIMODAL (fix -2014 nettoyage cles / palier 1 supprime / palier affiche / chien de garde stop natif / anti-fantomes : confirmation 2x + age min 20s + verrou d'entree + P&L reel Binance — decret Calvin 03/09) — ${MODE.toUpperCase()} — capital $${CAPITAL_START}`);
  logLine(`\u{1F4C8} 3.14e-Cadre R TRIMODAL — cles API nettoyees (NFKC + ASCII imprimable seul : fin du -2014) + alerte rouge si Binance refuse — paliers de mise : palier 1 (50-100$) SUPPRIME, plancher a 100-175$ — chien de garde stop natif (re-pose 1x/min si position nue) — anti-fantomes : une fermeture native exige 2 absences consecutives de positionRisk + 20s d'age + aucune entree en vol ; P&L lu sur /fapi/v1/userTrades — partiel : RANGE +0.5R / tendance +1R / volatil +0.4R — bonus : partiel +15% puis break-even, verrou 6h apres perte — prises partielles enregistrees dans l'historique et /stats.csv — filtre carnet 3:1 — cap 8 positions`);
