# Itachi — Serveur 3.12d « Champion » (Binance USDⓈ-M Futures)
...
## 2. Versions 3.12c / 3.12d — moteur et connexion, stratégie intacte

**3.12d — connexion depuis le dashboard** : onglets **TESTNET / MAINNET**, champs
API Key/Secret et bouton **🔐 Connecter** directement dans l'interface (architecture
v8). Plus besoin de variables d'environnement. Chaque connexion **remet la session
à zéro** et repart en **PAUSE**. Bouton **🔄 Reset** (stats/trades/capital à zéro,
positions Binance intactes et ré-adoptées). Les clés ne quittent jamais le serveur
(seul le préfixe s'affiche). Parité prouvée : **40/40 fonctions du moteur de trading
binairement identiques** au 3.12c.

**3.12c — moteur allégé.** Stratégie 3.12b strictement intacte — parité SHA-256 vérifiée sur les 27 fonctions
...
**Variables d'environnement — TOUTES OPTIONNELLES depuis le 3.12d** (le mode et
les clés se posent dans le dashboard) :

| Variable | Défaut | Rôle |
|---|---|---|
| `BINANCE_MODE` | `testnet` | Optionnelle — pré-sélectionne juste l'onglet au boot |
| `BINANCE_API_KEY` / `BINANCE_API_SECRET` | vide | Optionnelles — si présentes : **reconnexion automatique après un redéploiement/crash Railway** (recommandé pour du 24/7 sans surveillance). Sinon : recoller les clés dans le dashboard à chaque redémarrage — les **stops natifs Binance** protègent les positions pendant ce temps |
| `CAPITAL` | `1000` | Capital de référence ($) pour kill switch (−25 %) et exposition — mettre le solde réel |
| `PORT` | injecté par Railway | Port HTTP du dashboard |

**Clé API Binance** : permissions **Reading + Futures uniquement** (jamais retrait),
IP statiques du service Railway whitelistées.
...
- **Connexion** : onglet TESTNET ou MAINNET → coller API Key + Secret → **🔐 Connecter**
  (mainnet demande une confirmation). La session repart à zéro, en PAUSE. Le bandeau
  affiche « 🔐 Connecté MODE — clé XXXXXXXX… ».
- **🔄 Reset** : remise à zéro stats/trades/capital à la demande (Binance intact).
...
| Fichier | SHA-256 |
|---|---|
| `server.js` (3.12d) | `87e4ad976b9694b287f37d58ae29a195f572504cca890c89aa9bcd5c0155ad0d` |
...
- **3.12d** (2026-07-20) — connexion testnet/mainnet + clés dans le dashboard, Reset session (parité moteur 40/40 vs 3.12c).
